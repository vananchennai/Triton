...
if(isset($_SESSION["dbhostname"]) && isset($_SESSION["dbusername"]) && isset($_SESSION["dbpassword"]) && isset($_SESSION["databname"])) 
{
 $host=$_SESSION["dbhostname"];
 $uid=$_SESSION["dbusername"];
 $pass=$_SESSION["dbpassword"];
 $databname=$_SESSION["databname"];
define("HOST", $host); // The host you want to connect to.
define("USER", $uid); // The database username.
define("PASSWORD", $pass); // The database password. 
define("DATABASE", $databname); // The database name.



//$mysqli = new mysqli(HOST, USER, PASSWORD, DATABASE, 0,'/var/run/mysqld/mysqld.sock');
...
$pass=$_SESSION["dbpassword"];
$databname=$_SESSION["databname"];
//echo $host."hostname".$uid."user id".$pass."pass";
// $str ="";
// $data=array();
...
// list($host, $uid, $pass) = explode('~',trim($str));
//fclose($file);
