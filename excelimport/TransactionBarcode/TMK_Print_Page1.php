	...
	$_SESSION['currentyear']=date('Y');
		...
		unset($ASNNo);
		  ...
		  $DutyQuery= "SELECT * FROM tkm_invoice_detail_view WHERE vTKM_VchNo='".$_POST['bulkmasters']."'";
				...
				$gt_words_in_rupees = $obj_no_to_words->convert_number_to_words( $DutyDetails['vHMIL_GrandTotal']);//+$DutyDetails['VATCST_Amt']);
		...
		$flag = 0;
		 while($result = mysql_fetch_array($Invoiceresult)){
				...
				$BarcodeValue .= $result["vHMIL_GrandTotal"]."\t";
				$BarcodeValue .= "8481.80.90";//$result["TariffNo"];
				$BarcodeValue .= $result['vTKM_ExciseAmount']."\t";
				$BarcodeValue .= "0.00"."\t";
				$BarcodeValue .= "0.00"."\t";
				//$BarcodeValue .= "0.00"."\t";
				$BarcodeValue .= $result["vTKM_CSTAmt"]."\t";
