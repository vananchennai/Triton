<table id="default" style=" height:10px; display:none;" >
            <tr>
                <td>
                                    ...
                                    ?>
                                      ...
                                      <td>
                                        <select  name="dsrlist" id="dsrlist">
                                            <?
                                            $que = mysql_query("SELECT f.PrimaryFranchise,d.Franchisecode,d.dsrcode FROM dsrcodemapping d LEFT JOIN franchisemaster f ON f.Franchisecode = d.Franchisecode ORDER BY d.Franchisecode");
                                            while( $record = mysql_fetch_array($que))
                                            {   
                                              echo "<option value=\"".$record['PrimaryFranchise']."~".$record['Franchisecode']."~".$record['dsrcode']."\">".$record['PrimaryFranchise']."~".$record['Franchisecode']."~".$record['dsrcode']."\n ";
                                            }
                                            ?>
                                        </select>
                                      </td>
						<!--****************************Table Creation*************************** -->
		<td>
			<select  name="Invoicelist" id="Invoicelist">
				<?
				$que = mysql_query("SELECT Distinct(Invoice_No),Invoice_Date FROM `sales_invoice_details`");
				while( $record = mysql_fetch_array($que))
				{
				echo "<option value=\"".$record['Invoice_No']."~".$record['Invoice_Date']."\n "; 
				}
				?>
			</select>
		</td>
