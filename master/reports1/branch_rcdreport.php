...
//This block to authenticate the user whether he has the rights to access this page 
	$pagename = "Retailer Category Detailed";
...
if (isset($_POST['Excel'])) {
	header('Location:Export_rcdreport.php');
	echo '<META HTTP-EQUIV="Refresh" Content="0; URL=Export_rcdreport.php">';
	exit;
}
...
<title><?php echo $_SESSION['title']; ?> |&nbsp;Retailer Category Detailed Report</title>
</head>
...
LEFT JOIN franchisemaster f on (Franchisecode= SPLIT_STR(r.RetailerCode,'-',1) ) ;");
                    ...
                    <div style="width:930px; height:auto;   min-height: 180px; padding-bottom:8px; margin-top:8px; float:left; margin-left:10px;">
                        ...
                        <div style="width:930px; height:25px; float:left;  margin-left:0px;" class="head">
                            <p>Retailer Category Detailed Report </p>
                        </div>
                        <!-- main row 1 start-->     
                                     ...
                                     <select name='rc1[]' id = 'rc1' multiple="multiple" >
										   ...
										   $list1 = mysql_query($qry);
                                            while ($row_list1 = mysql_fetch_assoc($list1)) {
                                                ...
                                                if ($row_list1['retailercategory1'] == $Productgroup_select) {
                                                    $selected = ' selected ';
                                                }
                                                ?>
                                ...
                                <div style="float:left;width:400px">
                                 <div style="width:145px; height:30px; float:left; margin-top:5px; margin-left:45px;">
                                  ...
                                  <div style="width:145px; height:30px; float:left; margin-top:5px; margin-left:45px;">
                                    ...
                                    <div style="width:190px; height:auto;  float:left;  margin-top:5px; margin-left:3px;">
                                        <select name='franchise[]' id='franchise' onChange="drpfuncretailer();" multiple="multiple" >
                           ...
                           <table align="center"  bgcolor="#ffffFF" border="1" width="900px" cellpadding="20%">
                                <tr class="record_header" style="white-space:nowrap;">
                                   ...
                                   <td style="font-weight:bold; width:auto; text-align:center;">Franchisee Name</td>
                                    ...
                                    <td style="font-weight:bold; width:auto; text-align:center;">Voucher No.</td>
                                    <td style="font-weight:bold; width:auto; text-align:center;">Voucher Date</td>
                                ...
                                if (isset($_POST['Get'])) 
								{
									...
									$qry ="CALL r_RCDreport($dbfromdatefrom,$dbtodateto,$grystrres);";
								...
								// Log information
									...
									for($i=$startpoint;$i<$limit+$startpoint;$i++)
									{
										if($record[$i]['regionname']!=NULL)
										{
											...
											echo '<td>' . $record[$i]['salesdate'] . '</td>';											
											echo '<td>' . $record[$i]['vtype'] . '</td>';
											...
											echo '<td>' . $record[$i]['rcategory'] . '</td>';
											echo '<td>' . $record[$i]['rclass'] . '</td>';
											echo '<td>' . $record[$i]['franchiseeme'] . '</td>';
											echo '<td>' . $record[$i]['geographical'] . '</td>';
											echo '<td>' . $record[$i]['rc1'] . '</td>';
											echo '<td>' . $record[$i]['rc2'] . '</td>';
											echo '<td>' . $record[$i]['rc3'] . '</td>';
											echo '<td>' . $record[$i]['actualqty'] . '</td>';
											echo '</tr>';
										}
