...
//To Select All the DSR and their selected values based on PrimaryFranchisee
function drpfuncdsr_primary()
{
	var e = document.getElementById("primaryfranchise");
	var RetailerCodelist = document.getElementById('dsrlist');	
	var RetailerName = document.getElementById('dsr');	
	RetailerName.options.length = 0;
		for (j=0;j<e.options.length;j++) {
			if (e.options[j].selected) {
			var franchisecode = e.options[j].value;
   ...
   RetailerName.options.add(new Option('.All DSR.','0'));
	var usedNames = {};
	$("select[id='dsr'] > option").each(function () {
	if(usedNames[this.text] || this.text=='') {
	$(this).remove();
	} else {
	usedNames[this.text] = this.value;
...
//To Select All the DSR and their selected values based on Franchisee
function drpfuncdsr_franchisee()
{
	var e = document.getElementById("franchise");
	var RetailerCodelist = document.getElementById('dsrlist');	
	var RetailerName = document.getElementById('dsr');	
...
//To Select All the dsr
function alldsr()
{
	
	var e = document.getElementById("dsr");
	for (j=0;j<e.options.length;j++) {
	if (e.options[j].selected) {
	var franchisecode = e.options[j].value;
		if(franchisecode==0)
