...
//This block to authenticate the user whether he has the rights to access this page 
	$pagename = "Sub-Product Wise Sales Report";
...
if (isset($_POST['Excel'])) {
	header('Location:Export_spwswsreport.php');
    echo '<META HTTP-EQUIV="Refresh" Content="0; URL=Export_spwswsreport.php">';
    exit;
}
...
<title><?php echo $_SESSION['title']; ?> |&nbsp;Sub-Product Wise Stockist Wise Sales Report </title>
            ...
            <div style="width:950px; height:auto; float:left; margin-bottom:10px; margin-left:15px;" id="center">  
                ...
                <div style="width:930px; height:auto;   min-height: 180px; padding-bottom:8px; margin-top:8px; float:left; margin-left:10px;">
					<div style="width:930px; height:25px; float:left;  margin-left:0px;" class="head">
                            <p>Sub-Product Wise Stockist Wise Sales Report</p>
                        </div>
                       ...
                       <div style="width:917px; <?php echo $table_data_height ?> padding-bottom:8px; margin-top:20px; float:left; margin-left:10px; overflow:auto;" class="grid">
                                ...
                                if (isset($_POST['Get'])) 
								{
									...
									if(isset($_POST['branch'])){
									 ...
									 if(isset($_POST['primaryfranchise'])) {
	                                    ...
	                                    foreach ($_POST['primaryfranchise'] as $selectedOption3){
											...
											{
											$pfname =  $pfname. "r.pd_code = '" .$selectedOption3."'  OR ". "\n";	
											}else{
												$pfname = $pfname1;
												break;
											}
								  	...
								  	if(isset($_POST['franchise'])) 
				                  	{
					                    ...
					                    $fname=NULL;
	  									foreach ($_POST['franchise'] as $selectedOption2){
	  										...
	  										{
	  										 $fname = $fname . "r.franchisecode = '" .$selectedOption2."'  OR ". "\n";
	  										}else{	
					                        $fname = $fname1;
					                        break;
					                      }
									...
									foreach ($_POST['productcode'] as $selectedOption4){
										...
										{
										 $pcode=  $pcode. "r.productcode = '" .$selectedOption4."'  OR ". "\n";
										}
									}
									$reg =	substr($reg, 0, -3);
									$brn =	substr($brn, 0, -4);	
									...
									if($pfname!=NULL)
									{
										...
										$frnfilter = $frnfilter. "(".$pfname.") AND ";
									}
									if($fname!=NULL)
									{
										$grystr=$grystr. "(".$fname.") AND ";
										$frnfilter = $frnfilter. "(".$fname.") AND ";
									}
									if($pgrp!=NULL)
									{
									...
									if($pcode!=NULL)
									{
									 ...
									 $frnfilter = substr($frnfilter, 0, -4);
									if($_POST['SRP_FromDate']!="" && $_POST['SRP_ToDate']!="")
									{
										...
										$dbfromdatefrom=date('Y-m-d',strtotime($_POST['SRP_FromDate']));
											...
											if($frnfilter!=NULL)
											{
												$frnfilter = str_replace("r.franchisecode", "Franchisecode", $frnfilter);
												$frnfilter = str_replace("r.pd_code", "PrimaryFranchise", $frnfilter);
												$frnfilter= str_replace("'","''",$frnfilter);
												$frnfilter= "'".$frnfilter."'";
										...
										$dbtodateto="'".$dbtodateto."'";
										 $qry ="CALL r_spwswsreport($dbfromdatefrom,$dbtodateto,$grystrres,$frnfilter);";
							   ...
							   ?>
			                    ...
			                    // Get Records from the table
                                   ...
                                   while ($row = mysql_fetch_row($qry_exec)) {
