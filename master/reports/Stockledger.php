...
//This block to authenticate the user whether he has the rights to access this page 
	...
	// Export function 
		...
		{
			header('Location:Export_stock.php');
			echo '<META HTTP-EQUIV="Refresh" Content="0; URL=Export_stock.php">';
			exit;
		}
		...
		<script type="text/javascript">
			document.location='Stockledger.php';
		</script>
		<?
		}
	...
	<title><?php echo $_SESSION['title']; ?> |&nbsp;Stock Ledger  Report</title>
</head>
            ...
            <div style="width:950px; height:auto; float:left; margin-bottom:10px; margin-left:15px;" id="center">  
				...
				<form method="POST" action="<?php // $_PHP_SELF            ?>">
						...
						<div style="width:800px; height:auto; padding-bottom:5px; float:left; " class="cont">
							...
							<div style="float:left;width:400px">
								...
								<div style="width:190px; height:auto;  float:left;  margin-top:5px; margin-left:3px;">
									<input type="text" name="SRP_FromDate" id="stock_frdate" value="<?php echo $_POST['SRP_FromDate']; ?>" readonly />
								</div>
							...
							<div style="float:left;width:400px">
								...
								<div style="width:190px; height:auto;  float:left;  margin-top:5px; margin-left:3px;">
									<input type="text" name="SRP_ToDate" id="stock_todate" value="<?php echo $_POST['SRP_ToDate']; ?>" readonly />
								</div>
								<? if (($authen_row['usertype'])== 'Others') { 
                                        require_once 'first_div_element.php';
					...
					<?php $table_data_height = '';
						if (isset($_POST['Get'])) 
						{
							$table_data_height = " height:500px; ";
						} 
					?>
					<div style="width:917px; <?php echo $table_data_height ?> padding-bottom:8px; margin-top:20px; float:left; margin-left:10px; overflow:auto;" class="grid">
						...
						<table align="center" border="1" width="900px" cellpadding="20%"  bgcolor="#ffffFF">
							...
							<tr class="record_header" style="white-space:nowrap;">
                                ...
                                <td style="font-weight:bold; text-align:center;">Distributor Code</td>
                                <td style="font-weight:bold; text-align:center;">Distributor Name</td>
                                <td style="font-weight:bold; text-align:center;">Product Group</td>                                    
                               <!-- <td style="font-weight:bold; text-align:center;">Product Segment</td>  
                                <td style="font-weight:bold; text-align:center;">Product Type</td> -->
                                <td style="font-weight:bold; text-align:center;">Product Code</td> 
                                <td style="font-weight:bold; text-align:center;">Product Description</td>                                       
                                <td style="font-weight:bold; text-align:center;">Opening Stock</td>
                                <td style="font-weight:bold; text-align:center;">Receipt</td>
                                <td style="font-weight:bold; text-align:center;">Issues</td>
                                <td style="font-weight:bold; text-align:center;">Closing Stock</td>
                              <!--  <td style="font-weight:bold; text-align:center;">Stock value</td>-->
                                <td style="font-weight:bold; text-align:center;">Run date</td>
							</tr>
							...
							<?php
								if (isset($_POST['Get'])) 
								{
									...
									foreach ($_POST['Productgroup'] as $selectedOption3)
									{
										...
										{
											$pgrp =  $pgrp. "pgroupcode = '" .$selectedOption3."'  OR ". "\n";		
										}
									}
									...
									foreach ($_POST['productcode'] as $selectedOption6)
									{
										...
										{
											$pcode =  $pcode. "stockoutput.pc = '" .$selectedOption6."'  OR ". "\n";
										}
									}
									...
									{
										$SL_ToDate=date("Y-m-d",strtotime($_POST['SRP_ToDate'])) ;
										$SL_FromDate=date('Y-m-d',strtotime($_POST['SRP_FromDate']));
										$delqry="delete  from tempdate";
										$dateqry1=mysql_query($delqry);
										$insertqry="insert into tempdate values('".$SL_FromDate."','" . $SL_ToDate . "','sunday','5')";
										$dateqry1=mysql_query($insertqry);
										if($grystrres!=NULL)
										{
										...
										$SL_ToDate="'".$SL_ToDate."'";
										$qry ="CALL stockreport($SL_FromDate,$SL_ToDate,$grystrres);";
										$_SESSION['form_controls'] = $qry ; 
										//$_SESSION['rundate']=$_POST['SRP_ToDate'];
									}
									else
									{
											?>
								...
								while( $record[] = mysql_fetch_array($qry_exec));
									for($i=$startpoint;$i<$limit+$startpoint;$i++)
									{
										...
										{
										if(!empty($record[$i]['rundate']))
											{
												$rundate=date("d/m/Y",strtotime($record[$i]['rundate']));
											}
											else
											{
												$rundate="";
										...
										echo '<td>' . $record[$i]['opstock'] . '</td>';
										echo '<td>' . $record[$i]['ReceivedQty'] . '</td>';
										echo '<td>' . $record[$i]['IsseudQty'] . '</td>';
										echo '<td>' . $record[$i]['StockonHand'] . '</td>';
									//	echo '<td>' . $record[$i]['stockvalue'] . '</td>';
										echo '<td>' . $rundate . '</td>';
										echo '</tr>';
										}
									}
