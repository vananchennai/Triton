...
if(isset($_POST['Save']))
{
	...
	$Region = $_POST['Region'];
	 ...
	 if(!empty($_POST['FranchiseCode'])&&!empty($_POST['FranchiseName'])&&!empty($_POST['address'])&&!empty($_POST['country'])&&!empty($_POST['state'])&&!empty($_POST['designation'])&&!empty($_POST['pincode'])&&!empty($_POST['telephoneno'])&&!empty($_POST['branch'])&&!empty($_POST['contactperson'])&&!empty($_POST['email'])&&!empty($_POST['mobileno']))
	 {
	 $p1=strtoupper( preg_replace('/\s+/', '',$post['FranchiseCode']));
	$p2=strtoupper( preg_replace('/\s+/', '',$post['FranchiseName']));
	$cnduplicate=0;
	$repqry="SELECT REPLACE( `FranchiseCode` ,  ' ',  '' ) AS FranchiseCode, REPLACE(  `FranchiseName` ,  ' ',  '' ) AS FranchiseName FROM franchisemaster where FranchiseName = '".$p2."' or FranchiseName = '".$post['FranchiseName']."' or FranchiseCode = '".$p2."' or FranchiseCode = '".$post['FranchiseName']."' or FranchiseCode = '".$p1."' or FranchiseCode = '".$post['FranchiseCode']."' or FranchiseName = '".$p1."' or FranchiseName = '".$post['FranchiseCode']."'";
	$repres= mysql_query($repqry) or die (mysql_error());
	$cnduplicate=mysql_num_rows($repres);
		if($cnduplicate>0 || ($post['FranchiseCode']==$post['FranchiseName']))
		{
		?>
            <script type="text/javascript">
	...
	for($cnt=0;$cnt<$result;$cnt++)
     {
		...
		$spost1['InsertDate']=date("Y/m/d");
	  	$spost1['Deliverydae']=date("Y/m/d");
			...
			/* if($spost['Masters']=="vehiclemakemaster")
			{		
				$franqry= mysql_query("SELECT `MakeNo` FROM `vehiclemakemaster`") or die (mysql_error());
				  while($frqry = mysql_fetch_array($franqry))
				  {
					  $spost['Code']=$frqry['MakeNo'];
					  $news->addNews($spost,$stname);
				  }
			}
			
			else if($spost['Masters']=="vehiclemodel")
			{
				$franqry1= mysql_query("SELECT `modelcode` FROM `vehiclemodel`") or die (mysql_error());
				  while($frqry1 = mysql_fetch_array($franqry1))
				  {
					  $spost['Code']=$frqry1['modelcode'];
					  $news->addNews($spost,$stname1);
				  }
			}
						
			else if($spost['Masters']=="vehiclesegmentmaster")
			{
				$franqry2= mysql_query("SELECT `segmentcode` FROM `vehiclesegmentmaster`") or die (mysql_error());
				  while($frqry2 = mysql_fetch_array($franqry2))
				  {
					  $spost['Code']=$frqry2['segmentcode'];
					  $news->addNews($spost,$stname2);
				  }
			}
			 
		    else if($spost['Masters']=="retailermaster")
			{
				$franqry3= mysql_query("SELECT `RetailerCode` FROM `retailermaster`") or die (mysql_error());
				  while($frqry3 = mysql_fetch_array($franqry3))
				  {
					  $spost['Code']=$frqry3['RetailerCode'];
					  $news->addNews($spost,$stname3);
				  }
			} */
			
			if($spost['Masters']=="retailercategory")
			{
				$franqry4= mysql_query("SELECT `CategoryCode` FROM `retailercategory`") or die (mysql_error());
				  while($frqry4 = mysql_fetch_array($franqry4))
				  {
					  $spost['Code']=$frqry4['CategoryCode'];
					  $news->addNews($spost,$stname4);
				  }
			}
			/*else if($spost['Masters']=="failuremode")
			{
				$franqry5= mysql_query("SELECT `failuremode` FROM `failuremode`") or die (mysql_error());
				  while($frqry5 = mysql_fetch_array($franqry5))
				  {
					  $spost['Code']=$frqry5['failuremode'];
					  $news->addNews($spost,$stname5);
				  }
			}
			else if($post1['Masters']=="pricelistlinkinggrid")
			{
				$franqryp= mysql_query("SELECT `PriceListCode` FROM `pricelistlinkinggrid`") or die (mysql_error());
				  while($frqryp = mysql_fetch_array($franqryp))
				  {
					  $spost['PriceListCode']=$frqryp['pricelistlinkinggrid'];
					  $news->addNews($spost1,$tnameprice);
				  }
			}*/
			else if($spost['Masters']=="productgroupmaster")
			{
			...
			/* else if($spost['Masters']=="productsegmentmaster")
			{
				$franqry7= mysql_query("SELECT `ProductSegmentCode` FROM `productsegmentmaster`") or die (mysql_error());
				  while($frqry7 = mysql_fetch_array($franqry7))
				  {
					  $spost['Code']=$frqry7['ProductSegmentCode'];
					  $news->addNews($spost,$stname7);
				  }
			}
			else if($spost['Masters']=="producttypemaster")
			{
				$franqry8= mysql_query("SELECT `ProductTypeCode` FROM `producttypemaster`") or die (mysql_error());
				  while($frqry8 = mysql_fetch_array($franqry8))
				  {
					  $spost['Code']=$frqry8['ProductTypeCode'];
					  $news->addNews($spost,$stname8);
				  }
			} */
			else if($spost['Masters']=="productuom")
			{
...
/* 			else if($spost['Masters']=="productmaster")
			{
				$franqry10= mysql_query("SELECT `ProductCode` FROM `productmaster`") or die (mysql_error());
				  while($frqry10 = mysql_fetch_array($franqry10))
				  {
					  $spost['Code']=$frqry10['ProductCode'];
					  $news->addNews($spost,$stname10);
				  }
			} */
			/* else if($spost['Masters']=="productwarranty")
			{
				$franqry11= mysql_query("SELECT `ProductCode` FROM `productwarranty`") or die (mysql_error());
				  while($frqry11 = mysql_fetch_array($franqry11))
				  {
					  $spost['Code']=$frqry11['ProductCode'];
					  $news->addNews($spost,$stname11);
				  }
			} */
/* 			else if($spost['Masters']=="productmapping")
			{
				$franqry12= mysql_query("SELECT `ProductCode` FROM `productmapping`") or die (mysql_error());
				  while($frqry12 = mysql_fetch_array($franqry12))
				  {
					  $spost['Code']=$frqry12['ProductCode'];
					  $news->addNews($spost,$stname12);
				  }
			} */
			/* else if($spost['Masters']=="servicemaster")
			{
				$franqry13= mysql_query("SELECT `Productcode` FROM `servicemaster`") or die (mysql_error());
				  while($frqry13 = mysql_fetch_array($franqry13))
				  {
					  $spost['Code']=$frqry13['Productcode'];
					  $news->addNews($spost,$stname13);
				  }
			}
			else if($spost['Masters']=="oemmaster")
			{
				$franqry14= mysql_query("SELECT `oemcode` FROM `oemmaster`") or die (mysql_error());
				  while($frqry14 = mysql_fetch_array($franqry14))
				  {
					  $spost['Code']=$frqry14['oemcode'];
					  $news->addNews($spost,$stname14);
				  }
			} */
			else if($spost['Masters']=="schememaster")
			{
				$franqry15= mysql_query("SELECT `schemecode` FROM `schememaster`") or die (mysql_error());
				  while($frqry15 = mysql_fetch_array($franqry15))
				  {
					  $spost['Code']=$frqry15['schemecode'];//schememaster
					  $news->addNews($spost,$stname15);
				  }
			}
		else
		{
...
$post['Region'] = $smallfetch['RegionCode'];
	 ...
	 if(!empty($_POST['FranchiseCode'])&&!empty($_POST['FranchiseName'])&&!empty($_POST['designation'])&&!empty($_POST['address'])&&!empty($_POST['country'])&&!empty($_POST['state'])&&!empty($_POST['designation'])&&!empty($_POST['pincode'])&&!empty($_POST['telephoneno'])&&!empty($_POST['branch'])&&!empty($_POST['contactperson'])&&!empty($_POST['email'])&&!empty($_POST['mobileno'])&&!empty($_POST['Region']))
	 { 
			$codenamedcheck=0;
		if($_SESSION['frsessionval']!=$FranchiseName)
		{
		$p2=strtoupper( preg_replace('/\s+/', '',$post['FranchiseName']));
		$repqry="SELECT REPLACE(  `FranchiseName` ,  ' ',  '' ) AS FranchiseName  FROM  `franchisemaster` where FranchiseName = '".$p2."' or FranchiseName = '".$post['FranchiseName']."' or FranchiseCode = '".$p2."' or FranchiseCode = '".$post['FranchiseName']."'";
		$repres= mysql_query($repqry) or die (mysql_error());
...
$stringData =$myrecord[0]."\t ;".$myrecord[1]."\t ;".$myrecord[2]."\t ;".$myrecord[3]."\t ;".$myrecord[4]."\t  ;".$myrecord[5]."\t ;".$myrecord[6]."\t ;".$myrecord[7]."\t ;".$myrecord[8].";\t". $testtempp."\t ;".$statetempp."\t ;".$regiontempp."\t ;".$myrecord[14]." ;".$myrecord[15]." ;\t\n";//
							...
							?>
                             ...
                             <!--Row4 -->  
                              ...
                              <div style="width:145px; height:65px;  float:left;  margin-top:5px; margin-left:3px;">
                                 <textarea rows="4" cols="14" name="address" id="address" style="text-transform:uppercase;" onKeyPress="return validateAddress(event)" onChange="return trim(this)" maxlength="250"><? echo $address?></textarea>
                               </div>
                         <div style="width:145px; height:30px; float:left; margin-top:5px; margin-left:3px;">
