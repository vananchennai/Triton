...
//This block to authenticate the user whether he has the rights to access this page 
  $pagename = "Fast Moving GUI Report";
...
unset($_SESSION['form_controls']);
    ...
    <script type="text/javascript">
        document.location='FastMovingGuiReport.php';
    </script>
    <?
}
...
<title>SSV&nbsp;|&nbsp;Fast Moving GUI Report</title>
</head>
                  ...
                  if($_POST['SRP_FromDate']!="" && $_POST['SRP_ToDate']!="")
                  {
                    ...
                    $dbfromdatefrom=date('Y-m-d',strtotime($_POST['SRP_FromDate']));
                      if($gui_query!=NULL && $group_by!=NULL)
                      {
                         ...
                         $gui_query = $gui_query." AND  rs.salesdates between $dbfromdatefrom AND $dbtodateto group by ProductGroup order by Quantity Desc";
                       // echo $gui_query; 
                         $gui_result = mysql_query($gui_query);
                         $numrows = mysql_num_rows($gui_result);
                         $dis_productcode = "GUI Fast Moving Product Group Report";//.$dis_franchisename;
