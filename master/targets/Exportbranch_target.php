...
$table .= '"Region Code","Region Name","Branch Code","Branch Name","MonthYear","Product Type Code","Product Type Name","Target",';
$table .="\n"; 
...
for ($i = 0; $i < $columns_total; $i++) {
	if($i == 0){
		$bnamesel = "select branchname,RegionName,region from view_branch1 where branchcode='".$row[0]."'";
		$bnameqry = mysql_query($bnamesel);
		$bnamerow = mysql_fetch_array($bnameqry);
		$table .='"'.$bnamerow["region"].'",';
		$table .='"'.$bnamerow["RegionName"].'",';
	}
$table .='"'.$row["$i"].'",';
	if($i== 0){
		
		$table .='"'.$bnamerow["branchname"].'",';
	}
	if($i==2){
		$prosel = "select ProductTypeName from producttypemaster where ProductTypeCode='".$row[2]."'";
		$proqry = mysql_query($prosel);
		$prorow = mysql_fetch_array($proqry);
		$table .='"'.$prorow["ProductTypeName"].'",';
	}
}
$table .="\n";
}
// Download the file
$filename = "Branch Target-".date('Y-m-d').".csv";
